...
export default function Admin() {
  const { school: activeSchool } = useAuth();
  ...
  async function addSchool() {
    setSchoolMsg('');
    if (!schoolForm.name) {
      setSchoolMsg('School name is required');
      return;
    }
    try {
  ...
  async function deleteSchool(id, name) {
    const doubleConfirm = window.confirm(
      `WARNING: Are you sure you want to delete the school "${name}"?\n\nThis will PERMANENTLY erase all stock inventory, bills, customer accounts, purchase logs, return records, and users associated with this school. This action is irreversible!`
    );
      ...
      <div className="md:col-span-2 space-y-6">
        ...
        <div className="bg-white rounded-lg border border-black/5 shadow-sm p-5">
          <h2 className="font-medium mb-4 flex items-center gap-2">
            <Building2 className="text-black/50" size={20} />
            School List
          </h2>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg border border-black/5 shadow-sm p-5">
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={addSchool}
              className="w-full py-2.5 rounded-md text-white text-sm font-medium hover:opacity-95 transition-opacity"
