...
export default function Billing() {
  ...
  // Camera-based scanning using the browser's native BarcodeDetector (Chrome/Edge).
  // Falls back to a clear message on browsers that don't support it — the keyboard-
  // wedge USB scanner input above works everywhere regardless.
  useEffect(() => {
      ...
      <div className="max-w-md mx-auto">
        <div id="print-bill" className="bg-white border border-black/10 rounded-lg p-6 font-mono text-sm">
          <div className="text-center mb-3">
            {school?.logo_path && (
              <img src={school.logo_path} alt="" className="w-14 h-14 mx-auto mb-1 object-cover rounded-full" />
            )}
        ...
        <p className="text-xs text-black/40 mt-2 no-print">
          WhatsApp opens in a new tab/app with the bill ready to send. SMS opens your phone's messaging app (works on mobile devices).
        </p>
      </div>
    );
  }
