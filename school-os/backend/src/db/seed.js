...
import { pool, query, tenantContext } from "../config/db.js";
...
async function seed() {
  ...
  // Bypass RLS manually since tenants doesn't have it, but we need to insert tenants first
  const { rows: tRows1 } = await query(
    `INSERT INTO tenants (name, subdomain, contact_email) VALUES ('S.N Public School', 'snps', 'hello@snps.com') ON CONFLICT (subdomain) DO UPDATE SET name = EXCLUDED.name RETURNING id`
  );
  await query(
    `INSERT INTO tenants (name, subdomain, contact_email) VALUES ('Delhi Public School', 'dps', 'hello@dps.com') ON CONFLICT (subdomain) DO NOTHING`
  );
  const tenant1 = tRows1[0].id;
  
  // Run the rest of the seeding inside Tenant 1 context
  tenantContext.run(tenant1, async () => {
    // Seed super admin for Tenant 1
    const passwordHash = await bcrypt.hash(DEMO_PASSWORD, 10);
    await query(
      `INSERT INTO users (name, email, password_hash, role) VALUES ('Super Admin', 'admin@schoolos.com', $1, 'super_admin') ON CONFLICT DO NOTHING`,
      [passwordHash]
    );

    // Seed.sql must be run inside tenant context to respect RLS
    const sql = fs.readFileSync(path.join(__dirname, "seed.sql"), "utf-8");
    console.log("Seeding reference data for SN Public School...");
  ...
  // passwordHash already declared and calculated above
  ...
  for (const u of DEMO_USERS) {
    const { rows } = await query(
      `INSERT INTO users (name, email, password_hash, role)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (email, tenant_id) DO UPDATE SET name = EXCLUDED.name RETURNING id`,
  ...
  const cls = await query(`SELECT id FROM classes WHERE name = 'Class 8' LIMIT 1`);
  const sec = await query(`SELECT id FROM sections WHERE class_id = $1 LIMIT 1`, [cls.rows[0].id]);
  ...
  const { rows: studentRows } = await query(`SELECT id FROM students LIMIT 1`);
  const { rows: feeStructureRows } = await query(`SELECT id FROM fee_structure LIMIT 1`);
