...
// Assign a student to a bus (admin/office/principal/transport).
export async function assignStudentToBus(req, res) {
  const { student_id, bus_id, pickup_point } = req.body;
  if (!student_id || !bus_id) return res.status(400).json({ error: "student_id and bus_id are required." });

  // One active assignment per student — replace any existing one.
  await query(`DELETE FROM student_transport WHERE student_id = $1`, [student_id]);
  const { rows } = await query(
    `INSERT INTO student_transport (student_id, bus_id, pickup_point) VALUES ($1, $2, $3) RETURNING *`,
    [student_id, bus_id, pickup_point || null]
  );
  res.status(201).json({ assignment: rows[0] });
}

// List students riding a given bus (for the driver/admin roster).
export async function listBusStudents(req, res) {
  const { rows } = await query(
    `SELECT s.id, s.name, s.admission_no, st.pickup_point,
  ...
  try {
    await pool.query(
      `UPDATE buses 
       SET gps_device_id = $1 
       WHERE id = $2`,
      [gps_device_id || null, id]
    );
    res.json({ message: "Bus updated successfully" });
  } catch (err) {
    if (err.code === "23505") {
      return res.status(400).json({ error: "GPS Device ID is already assigned to another bus." });
    }
