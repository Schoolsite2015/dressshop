  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        {[
          { label: "Today's Sales", value: stats ? formatINR(stats.todaysSales) : '—', accent: 'maroon' },
          { label: "Today's Bills", value: stats?.todaysBills ?? '—', accent: '' },
          { label: 'Low Stock Alerts', value: stats?.lowStockAlerts ?? '—', accent: 'gold' },
          { label: 'Out of Stock', value: stats?.outOfStock ?? '—', accent: 'maroon' },
          { label: 'Total Stock', value: stats ? `${stats.totalStock.toLocaleString('en-IN')} items` : '—', accent: '' },
          { label: 'Revenue This Month', value: stats ? formatINR(stats.revenueThisMonth) : '—', accent: 'gold' },
        ].map((s) => (
          <div key={s.label} className="bg-white rounded-lg border border-black/5 shadow-sm p-4">
            <div className="text-xs uppercase tracking-wide text-black/40 mb-1">{s.label}</div>
            <div className="font-display text-xl font-semibold"
              style={{ color: s.accent === 'maroon' ? 'var(--color-maroon)' : s.accent === 'gold' ? 'var(--color-gold)' : 'inherit' }}>
              {s.value}
            </div>
          </div>
        ))}
