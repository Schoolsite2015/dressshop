...
import { Bus, Radio, MapPin, X } from "lucide-react";
    ...
    <DashboardShell title="Transport" subtitle="Live bus tracking and route management">
      ...
      {tab==="buses" && (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {(buses||[]).map(b=>(
            <div key={b.id} className="card p-5 hover:shadow-md transition-shadow relative">
              ...
              <div className="mt-3 pt-3 border-t border-indigo-50 text-xs text-indigo-400 space-y-1">
                ...
                <p>GPS ID: <span className="text-ink">{b.gps_device_id || "Not Linked"}</span></p>
                {busLocs[b.id] && (
      ...
      {editBus && (
        <EditBusModal
          ...
          onChanged={() => qc.invalidateQueries({ queryKey: ["buses"] })}
        />
      )}
    </DashboardShell>
  );
...
function EditBusModal({ bus, onClose, onChanged }) {
  const [gpsId, setGpsId] = useState(bus.gps_device_id || "");

  const mut = useMutation({
    mutationFn: () => api.put(`/transport/buses/${bus.id}`, { gps_device_id: gpsId }),
    onSuccess: () => {
      onChanged();
      onClose();
    }
    ...
    <div className="fixed inset-0 bg-ink/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm animate-slide-up" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <p className="font-display text-lg text-indigo-700">Edit Bus: {bus.number_plate}</p>
          <button onClick={onClose} className="p-1.5 hover:bg-indigo-50 rounded-lg"><X size={16} className="text-indigo-400" /></button>
        </div>
        
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-indigo-500 mb-1">GPS Device ID (IMEI)</label>
            <input 
              type="text" 
              value={gpsId} 
            ...
            />
            <p className="text-[10px] text-indigo-400 mt-1">Configure your tracker to send TCP packets to port 4005.</p>
          </div>
          
          {mut.isError && <p className="text-sm text-red-500">{mut.error?.response?.data?.error || "Failed to update bus."}</p>}
          
          <button onClick={() => mut.mutate()} disabled={mut.isPending} className="btn-primary w-full mt-2">
            {mut.isPending ? "Saving..." : "Save GPS ID"}
          </button>
        </div>
      </div>
    </div>
